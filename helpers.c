...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            {
                image [i][j].rgbtRed =  floor(((float)copyimage [i][j].rgbtRed + (float) copyimage [i][j+1].rgbtRed + (float) copyimage [i-1][j].rgbtRed + (float) copyimage [i-1][j+1].rgbtRed)/4);
                image [i][j].rgbtGreen =  floor(((float)copyimage [i][j].rgbtGreen + (float) copyimage [i][j+1].rgbtGreen + (float) copyimage [i-1][j].rgbtGreen + (float) copyimage [i-1][j+1].rgbtGreen)/4);
                image [i][j].rgbtBlue = floor(((float)copyimage [i][j].rgbtBlue + (float) copyimage [i][j+1].rgbtBlue + (float) copyimage [i-1][j].rgbtBlue + (float) copyimage [i-1][j+1].rgbtBlue)/4);
            }
            else if(i == 0)
            {
