...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            {
                image [i][j].rgbtRed =  round((float)copyimage [i][j].rgbtRed + (float) copyimage [i+1][j].rgbtRed + (float) copyimage [i-1][j].rgbtRed + (float) copyimage [i+1][j+1].rgbtRed + (float) copyimage [i-1][i+1].rgbtRed + (float) copyimage [i][j+1].rgbtRed)/6;
